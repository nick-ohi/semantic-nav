...
int main(int argc, char** argv)
{
    ...
    for (;;)
    {
        ...
        printf("labels type = %u\n",labels.type());

        /* get the contours for displaying */
        seeds->getLabelContourMask(mask, false);
        result.setTo(Scalar(0, 0, 255), mask);

        // Find superpixel centers and display
        unsigned int numSuperpixelsActual = seeds->getNumberOfSuperpixels();
        std::vector<cv::Moments> superpixelMoments;
        std::vector<cv::Point2f> superpixelCenters;
        cv::Mat singleSuperpixelFrame;
        superpixelMoments.resize(numSuperpixelsActual);
        superpixelCenters.resize(numSuperpixelsActual);
        for(unsigned int i=0; i<numSuperpixelsActual; i++)
        {
            singleSuperpixelFrame = cv::Mat(labels.size(), labels.type(), cv::Scalar(0));
            cv::inRange(labels, cv::Scalar(i), cv::Scalar(i), singleSuperpixelFrame);
            superpixelMoments.at(i) = cv::moments(singleSuperpixelFrame, true);
            superpixelCenters.at(i) = cv::Point2f(superpixelMoments.at(i).m10/superpixelMoments.at(i).m00, superpixelMoments.at(i).m01/superpixelMoments.at(i).m00);
            cv::circle(result, superpixelCenters.at(i), 4, cv::Scalar(0,255,0), -1, 8, 0);
            cv::circle(labels, superpixelCenters.at(i), 4, cv::Scalar(255), -1, 8, 0);
        }
